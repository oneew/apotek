...
import { FiFilter, FiSearch, FiPlus, FiTrash2, FiEdit2, FiCreditCard } from 'react-icons/fi';
...
export default function PromoVoucher() {
  ...
  const columns = [
    ...
    { 
        key: 'nilai_diskon', 
        label: 'Nilai Voucher',
        render: (val) => <span className="font-bold text-primary-700">Rp {new Intl.NumberFormat('id-ID').format(val)}</span>
    },
    { 
      key: 'tanggal_mulai', 
      label: 'Masa Berlaku',
  ...
  const fetchData = async () => {
    try {
      ...
      setData(res.data.data.filter(d => d.jenis_promo === 'Voucher'));
    } catch (err) {
      Swal.fire({ icon: 'error', title: 'Error', text: 'Gagal mengambil data voucher', confirmButtonColor: '#7F56D9' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
        try {
          ...
          Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Voucher berhasil dihapus', timer: 1500, showConfirmButton: false });
          fetchData();
        } catch (err) {
          Swal.fire({ icon: 'error', title: 'Error', text: 'Gagal menghapus voucher', confirmButtonColor: '#7F56D9' });
        }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Voucher berhasil diupdate', timer: 1500, showConfirmButton: false });
      } else {
        await axios.post('http://localhost:8080/api/master/promo', formData);
        Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Voucher berhasil ditambahkan', timer: 1500, showConfirmButton: false });
      }
      ...
      Swal.fire({ icon: 'error', title: 'Error', text: 'Gagal menyimpan voucher', confirmButtonColor: '#7F56D9' });
    }
  };
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-6 pb-12 p-6">
      <SectionHeader 
        ...
        subtitle="Kelola voucher potongan harga belanja"
      >
          <button 
      ...
      <ModalDialog
        ...
        title={editingId ? 'Edit Voucher Belanja' : 'Tambah Voucher Belanja'}
      >
        <form onSubmit={handleSubmit} className="flex flex-col">
          <div className="p-6 space-y-4">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700">Nama Voucher / Kode</label>
              <input 
                type="text" required
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-gray-700">Nilai Potongan (Rp)</label>
                  <input 
