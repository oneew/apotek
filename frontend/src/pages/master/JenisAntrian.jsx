...
import { FiArrowRight, FiEdit2, FiPlus, FiTrash2, FiUsers } from 'react-icons/fi';
import Swal from 'sweetalert2';

export default function JenisAntrian() {
  ...
  const [formData, setFormData] = useState({ id: null, nama_antrian: '', kode_prefix: '', status: 'Aktif' });

  const fetchItems = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8080/api/master/jenis-antrian');
  ...
  const handleOpenModal = (item = null) => {
    if (item) {
      ...
      setIsEditing(false);
      setFormData({ id: null, nama_antrian: '', kode_prefix: '', status: 'Aktif' });
    }
    setIsModalOpen(true);
  };

  const handleSave = async () => {
    if (!formData.nama_antrian || !formData.kode_prefix) {
      Swal.fire('Oops!', 'Nama Antrian dan Kode Prefix wajib diisi', 'warning');
      return;
    }

    setIsSubmitting(true);
    const url = isEditing 
      ? `http://localhost:8080/api/master/jenis-antrian/${formData.id}` 
      : 'http://localhost:8080/api/master/jenis-antrian';
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const response = await fetch(`http://localhost:8080/api/master/jenis-antrian/${id}`, { method: 'DELETE' });
        const result = await response.json();
        if (result.status) {
