...
export default function MasterPajak() {
  ...
  const [formData, setFormData] = useState({ id: null, nama_pajak: '', persentase: '', status: 'Aktif' });

  const fetchItems = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8080/api/master/pajak');
  ...
  const handleOpenModal = (item = null) => {
    if (item) {
      ...
      setIsEditing(false);
      setFormData({ id: null, nama_pajak: '', persentase: '', status: 'Aktif' });
    }
    setIsModalOpen(true);
  };

  const handleSave = async () => {
    if (!formData.nama_pajak || !formData.persentase) {
      Swal.fire({ title: 'Opps!', text: 'Lengkapi data pajak', icon: 'warning', customClass: { popup: 'rounded-3xl' } });
      return;
    }

    setIsSubmitting(true);
    const url = isEditing 
      ? `http://localhost:8080/api/master/pajak/${formData.id}` 
      : 'http://localhost:8080/api/master/pajak';
    ...
    try {
      ...
      Swal.fire({ title: 'Error!', text: 'Gagal menyimpan data', icon: 'error', customClass: { popup: 'rounded-3xl' } });
    } finally {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const response = await fetch(`http://localhost:8080/api/master/pajak/${id}`, { method: 'DELETE' });
        const result = await response.json();
        if (result.status) {
