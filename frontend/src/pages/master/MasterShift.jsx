...
export default function MasterShift() {
  ...
  const [formData, setFormData] = useState({ id: null, nama_shift: '', jam_mulai: '', jam_selesai: '', status: 'Aktif' });

  const fetchItems = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8080/api/master/shift');
  ...
  const handleOpenModal = (item = null) => {
    if (item) {
      ...
      setIsEditing(false);
      setFormData({ id: null, nama_shift: '', jam_mulai: '', jam_selesai: '', status: 'Aktif' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    if (!formData.nama_shift || !formData.jam_mulai || !formData.jam_selesai) {
      Swal.fire({ title: 'Opps!', text: 'Lengkapi seluruh data shift', icon: 'warning', customClass: { popup: 'rounded-3xl' } });
      return;
    }

    setIsSubmitting(true);
    const url = isEditing 
      ? `http://localhost:8080/api/master/shift/${formData.id}` 
      : 'http://localhost:8080/api/master/shift';
  ...
  const handleDelete = async (id) => {
    const confirm = await Swal.fire({
      title: 'Hapus Shift?',
      text: "Data jadwal kerja ini akan dihapus permanen.",
      icon: 'warning',
    ...
    if (confirm.isConfirmed) {
      try {
        const response = await fetch(`http://localhost:8080/api/master/shift/${id}`, { method: 'DELETE' });
        const result = await response.json();
        if (result.status) {
