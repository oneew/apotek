...
export default function MasterKategori() {
  ...
  const handleDelete = async (id) => {
    const confirm = await Swal.fire({
      ...
      text: "Tindakan ini tidak dapat dibatalkan dan dapat mempengaruhi klasifikasi produk.",
      icon: 'warning',
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Aksi Gagal',
          text: 'Data dilindungi atau terjadi kesalahan jaringan.'
        });
      }
    }
  };
  ...
  const columns = [
    { label: 'Kode Kategori', key: 'kode_kategori', width: '150px', render: (val) => <span className="font-semibold text-primary-700">{val || '-'}</span> },
    { label: 'Nama Kategori', key: 'nama_kategori', render: (val) => (
      <div className="flex flex-col">
        <span className="font-semibold text-gray-900 dark:text-gray-100 uppercase text-xs">{val}</span>
        <span className="text-[10px] text-gray-500 font-medium tracking-tight">REKORD KLASIFIKASI</span>
      </div>
    )},
    { label: 'Keterangan', key: 'keterangan', render: (val) => <span className="text-gray-500 font-medium text-[11px] leading-relaxed">{val || 'Tidak ada deskripsi tambahan...'}</span> },
  ...
  return (
    <div className="max-w-[1440px] mx-auto space-y-6 pb-20">
      <SectionHeader 
        title="Arsitektur Kategori" 
        subtitle="Mengatur dan menyusun taksonomi produk untuk manajemen inventaris yang akurat."
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 shadow-sm">
        <DataTable 
          ...
          searchPlaceholder="Filter repositori kategori..."
          primaryAction={{ label: "Tambah Kategori", onClick: () => handleOpenModal() }}
        />
      </div>
      ...
      <ModalDialog
        ...
        title={isEditing ? 'Ubah Signature Kategori' : 'Daftarkan Klasifikasi Baru'}
        subtitle="Menjaga konsistensi dalam klasifikasi produk."
        icon={<FiHash />}
        maxWidth="max-w-xl"
      >
        <div className="p-8 space-y-8">
          <div className="space-y-6">
            <div className="flex items-center gap-2 border-b border-gray-100 dark:border-gray-800 pb-3">
              ...
              <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Atribut Kategori</h3>
            </div>
            
            <div className="grid grid-cols-1 gap-5">
              <div className="space-y-1.5 focus-within:text-primary-600 transition-colors">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Kode Identifier</label>
                <input type="text" className="w-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-sm font-semibold focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all outline-none uppercase font-mono" value={formData.kode_kategori} onChange={(e) => setFormData({...formData, kode_kategori: e.target.value})} placeholder="Cth: CAT-999" />
              </div>
              <div className="space-y-1.5 focus-within:text-primary-600 transition-colors">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Label Klasifikasi</label>
                <input type="text" className="w-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-sm font-semibold focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all outline-none" value={formData.nama_kategori} onChange={(e) => setFormData({...formData, nama_kategori: e.target.value})} placeholder="Cth: Critical Care" />
              </div>
              <div className="space-y-1.5 focus-within:text-primary-600 transition-colors">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Narasi Internal</label>
                <textarea className="w-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-sm font-medium focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all outline-none resize-none" value={formData.keterangan} onChange={(e) => setFormData({...formData, keterangan: e.target.value})} placeholder="Metadata opsional..." rows="3" />
              </div>
            </div>
          </div>
